...
int main(int argc, char * argv[]) {
  ...
  //int max_electrons=100; // before it was used as array job on cluster it was this line
  int max_electrons=25; // use for array job on cluster
  std::stringstream outrootfilename;
  outrootfilename << "BON_iso_persensewire_cluster/isochron_" << BFieldValue << "_" << wireIDOfInterest << "_" << temperature  << "_"<< array_ID << "_.root";
...
//  cmp->SetMagneticField(0.,0.,0.0);
  ...
  */
  ...
  // need 72 of these histograms
  TH3D * iso_per_sensewire[72];
  int nbinsx=std::round((max_x_i-x_i)/(stepx/2.0))+1;
  double x_low_edge=x_i-(stepx/4.0);
  double x_high_edge=max_x_i+(stepx/4.0);
  for(int ih=0;ih<72;ih++){
    // thenbinsy depends on which wire it is because of edges
    double sensewire_y =whichYPos(ih)/10.0; // here y is Garfield y and not HELIX y. this is actually HELIX z.
    double histo_y_min=sensewire_y-2.0; // lowest for wire
    double histo_y_max=sensewire_y+2.0; // highest for wire
    if(ih==0){
      histo_y_min+=-1.0; // top of the DCT needs extra cm in the low edge
    }
    if(ih==71){
      histo_y_max+=1.0; // btm of the DCT needs extra cm in the high edge
    }
    int nbinsy=std::round((histo_y_max-histo_y_min)/(stepy/2.0))+1;
    double y_low_edge=histo_y_min-(stepy/4.0);
    double y_high_edge=histo_y_max+(stepy/4.0);
    std::cout << "histogram i with nbinsy, y_low_edge,y_high_edge     " << ih << "," << nbinsy << "," << y_low_edge << ", " << y_high_edge << std::endl;
    std::stringstream h3dnames;
    h3dnames.str( std::string());
    h3dnames.clear();
    // this is the wire of interest
    h3dnames << "sensewire_" << ih;
    iso_per_sensewire[ih] = new TH3D(h3dnames.str().c_str(),h3dnames.str().c_str(),1500,-0.5,14999.5,nbinsy,y_low_edge,y_high_edge,nbinsx,x_low_edge,x_high_edge);
  }
  ...
  for(int ix=0;ix<size_of_x_grid;ix++){
	...
	bool did_it_drift=driftline->DriftElectron(x_delt,y_delt,z_i,0);
        ...
        if(stat==-5){
          ...
          if(which_wireID>=0 && which_wireID<=71) iso_per_sensewire[which_wireID]->Fill(curr_sig,yendpoint,xendpoint);
      ...
      std::cout << "x = " << ix << ", y= " << iy << ", threadId = "<< omp_get_thread_num() << " and num_electrons was: " << num_electrons << std::endl; //i, j, omp_get_thread_num())
      auto endthread = std::chrono::high_resolution_clock::now();
      std::chrono::duration<double> elapsed_seconds_thread = endthread - startthread;
      std::cout << "Elapsed time for threadID: " << omp_get_thread_num() << " is " << elapsed_seconds_thread.count() << " seconds\n";
      //y_i=min_y_i;
      Outfile->cd();
  ...
  ydiffAll_h->Write();
  zdiffAll_h->Write();
  for(int ih=0;ih<72;ih++) iso_per_sensewire[ih]->Write();
  Outfile->Close();
  return 0;
}
