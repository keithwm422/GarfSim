...
//  const double rPotential= 175e-4;
 ...
 // We are essentially copying all of the computeDriftLines function inside of the ViewIsochrone Class
  ...
  const double tstep = 100.0; // 10 ns time steps when interpolating the resulting drift times
...
// everything else is one big for loop around the starting points
  ...
  for(int s=0;s<4;s++){
    ...
    graphNames.str("");
    graphNames << "sensewire_" << s+35;
    senses[s]->SetName(graphNames.str().c_str());
    graphNames.str("");
    graphNames << "Sense Wire " << s+35 << "; y(cm); z(cm)";
    senses[s]->SetTitle(graphNames.str().c_str());
    invsenses[s] = new TGraph2D();
    graphNames.str("");
    graphNames << "Invertedsensewire_" << s+35;
    invsenses[s]->SetName(graphNames.str().c_str());
    graphNames.str("");
    graphNames << "Inverse Isochrone Sense Wire " << s+35 << "; y(cm); z(cm)";
    invsenses[s]->SetTitle(graphNames.str().c_str());
  }
  for (int iter=0; iter<statusCodes.size();iter++) {
    if(statusCodes[iter]==36){
      for(const auto& drift: driftLines[iter]){
        senses[1]->AddPoint(drift[0],drift[1],drift[3]);
        invsenses[1]->AddPoint(drift[3],drift[1],drift[0]);
  ...
  int nbins=std::round(((double)(max_value)-(double)(min_value))/(step/2.0))+1;
  double low_edge=min_value-(step/4.0);
  double high_edge=max_value+(step/4.0);
  ...
  for(int dimension_index=0;dimension_index<3;dimension_index++){
    ...
    StartnbinsXYZ[dimension_index]=std::round(((double)(maxXYZ)-(double)(minXYZ))/(Startstepsizes[dimension_index]/2.0))+1;
    Startlow_edgesXYZ[dimension_index]=minXYZ-(Startstepsizes[dimension_index]/4.0);
    Starthigh_edgesXYZ[dimension_index]=maxXYZ+(Startstepsizes[dimension_index]/4.0);
  }
  ...
  for(int dimension_index=0;dimension_index<3;dimension_index++){
    ...
    // now fill my arrays for determining appropriate bins
    EndnbinsXYZ[dimension_index]=std::round(((double)(maxXYZ)-(double)(minXYZ))/(Endstepsizes[dimension_index]/2.0))+1;
    Endlow_edgesXYZ[dimension_index]=minXYZ-(Endstepsizes[dimension_index]/4.0);
    Endhigh_edgesXYZ[dimension_index]=maxXYZ+(Endstepsizes[dimension_index]/4.0);
  }
  ...
  outrootfilename << "DriftLineRKFTest.root";
