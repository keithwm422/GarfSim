...
//  const double rPotential= 175e-4;
  ...
  */
  ...
  const double dx = (nx > 1) ? (xmax - xmin) / (nx - 1) : 0;
  ...
  //std::ofstream outfile("generated_electric_field_grid.txt");
  std::ofstream outfile(outfileName.str());
  
  // Write header information expected by ComponentGrid format (e.g., "XYZ" format)
  outfile << "#" << nx << "  " << ny << " " << nz << "\n";
  outfile << "#" << xmin << "  " << xmax << "\n";
  outfile << "#" << ymin << "  " << ymax << "\n";
  outfile << "#" << zmin << "  " << zmax << "\n";
