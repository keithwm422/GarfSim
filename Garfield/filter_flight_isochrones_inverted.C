...
void filter_flight_isochrones_inverted() {

  TString dir = "flight_study_rootfiles";  
  ...
  Target = TFile::Open( "temperature_study_isochrones_output_293_inverted.root", "RECREATE" );
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
  ...
  auto g = new TGraph2D();
  g->SetTitle("myisochrone");
  g->SetName("myisochrone");
  //TH2D * isolow = new TH2D("iso293","iso293",31,-77.5,77.5,289,-289,289);
  TH2D * isolow = new TH2D("iso293_inverted","iso293_inverted",6000,-14997.5,14997.5,289,-289,289);
