...
int main(int argc, char * argv[]) {
  ...
  std::ofstream outputfile("electron_dirfttimes_coordinates_BON_with_distances_small.txt");
  outputfile << "num_electrons,average,stddev,y,z,x" << std::endl;
  ...
  while(z_i<=max_z_i){
    ...
    while(y_i<=max_y_i){
      ...
      while(keep_running){
        ...
        if(num_electrons>100 && TMath::Abs((new_average-running_average)/running_average)<=min_variation) keep_running=false;
        else{
