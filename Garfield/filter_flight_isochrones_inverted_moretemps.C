...
void filter_flight_isochrones_inverted_moretemps() {

  TString dir = "temp_study_moretemps";  
  ...
  Target = TFile::Open( "temperature_study_isochrones_output_inverted_moretemps.root", "RECREATE" );
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
  ...
  auto g = new TGraph2D();
  g->SetTitle("myisochrone293");
  g->SetName("myisochrone293");
  auto glow = new TGraph2D();
  glow->SetTitle("myisochrone289");
  glow->SetName("myisochrone289");
  auto ghigh = new TGraph2D();
  ghigh->SetTitle("myisochrone297");
  ghigh->SetName("myisochrone297");

  //TH2D * isolow = new TH2D("iso293","iso293",31,-77.5,77.5,289,-289,289);
  TH2D * isolow = new TH2D("iso289_inverted","iso289_inverted",6000,-14997.5,14997.5,578,-289,289);
  TH2D * isomid = new TH2D("iso293_inverted","iso293_inverted",6000,-14997.5,14997.5,578,-289,289);
  TH2D * isohigh = new TH2D("iso297_inverted","iso297_inverted",6000,-14997.5,14997.5,578,-289,289);

  for(int bint=1;bint<6001;bint++){
    for(int binz=1;binz<579;binz++){
      isolow->SetBinContent(bint,binz,-1000);
      isomid->SetBinContent(bint,binz,-1000);
      isohigh->SetBinContent(bint,binz,-1000);
    }
  }
  int iter_i=0;
      ...
      if ( obj->IsA()->InheritsFrom( TH1D::Class() ) ) {
        ...
        if ( thish ) {
          ...
          if(temperature==289){
            ...
            if(std::stod(splitted[4]) <0) isomid->SetBinContent(isomid->GetXaxis()->FindBin(-1.0*thish->GetMean()),isomid->GetYaxis()->FindBin(std::stod(splitted[2])),std::stod(splitted[4]));
            else isomid->SetBinContent(isomid->GetXaxis()->FindBin(thish->GetMean()),isomid->GetYaxis()->FindBin(std::stod(splitted[2])),std::stod(splitted[4]));              
          }
          else if(temperature==297){
            ghigh->SetPoint(iter_i,std::stod(splitted[4]),std::stod(splitted[2]),thish->GetMean());
            if(std::stod(splitted[4]) <0) isohigh->SetBinContent(isohigh->GetXaxis()->FindBin(-1.0*thish->GetMean()),isohigh->GetYaxis()->FindBin(std::stod(splitted[2])),std::stod(splitted[4]));
            else isohigh->SetBinContent(isohigh->GetXaxis()->FindBin(thish->GetMean()),isohigh->GetYaxis()->FindBin(std::stod(splitted[2])),std::stod(splitted[4]));              
          }
          //g->SetPoint(iter_i,std::stod(splitted[4]),std::stod(splitted[2]),thish->GetMean());
          //if(std::stod(splitted[4]) <0) isolow->SetBinContent(isolow->GetXaxis()->FindBin(-1.0*thish->GetMean()),isolow->GetYaxis()->FindBin(std::stod(splitted[2])),std::stod(splitted[4]));
          //else isolow->SetBinContent(isolow->GetXaxis()->FindBin(thish->GetMean()),isolow->GetYaxis()->FindBin(std::stod(splitted[2])),std::stod(splitted[4]));
