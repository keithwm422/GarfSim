...
void hiresstudy_combine() {

  TString dir = "hires_isochrone_study_02122025";  
  ...
  Target = TFile::Open( "hires_study_isochrones_output.root", "RECREATE" );
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
  ...
  ofstream pythonfile;
  pythonfile.open("iso293.csv");
  pythonfile << "average,y,z" << endl;
  // new TGraphto store things
  auto ghires = new TGraph2D();
  TH3D * statush= new TH3D("status","status",301,-301,301,62,-77.5,77.5,20,-20.5,-0.5);
  TH1D * statush2= new TH1D("status2","status2",200,-20.5,-0.5);
  TH2D * iso = new TH2D("iso293","iso293",62,-77.5,77.5,578,-289,289);

  ghires->SetTitle("myisochrone293");
  ghires->SetName("myisochrone293");
        ...
        if ( obj->IsA()->InheritsFrom( TH1D::Class() ) ) {
          ...
          if ( thish ) {
            ...
            //try to write the object to our new
              ghires->SetPoint(iter_i,std::stod(splitted[4]),std::stod(splitted[2]),thish->GetMean());
              iso->Fill(std::stod(splitted[4]),std::stod(splitted[2]),thish->GetMean());
              pythonfile << thish->GetMean() << "," << std::stod(splitted[4]) << "," << std::stod(splitted[2]) << endl;
