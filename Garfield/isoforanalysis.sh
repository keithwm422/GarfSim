...
for l in $(seq 0 1 2);
do
  for j in $(seq -75 2.5 75); # this is in millimeters
  do
    if (( $(bc <<< "$j == 0") )); then
      continue
    fi
    check
    echo "starting x is : ${j}"
    echo "zone is: ${l}"
    ...
    echo "max x is : ${k}"
    execute "./iso_gen_foranalysis $k $j 302 $l >> isoforanalysisredirects/simout_zone_${l}_${k}_${j}.txt 2>&1 &"
  done
  #now do the last + and minus 76.19
  for j in $(seq -76.19 152.38 76.19); # this is in millimeters
  do
    if (( $(bc <<< "$j == 0") )); then
      continue
    fi
