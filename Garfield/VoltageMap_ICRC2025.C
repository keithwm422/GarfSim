...
int main(int argc, char * argv[]) {
  ...
  efieldfilename << "efieldprofile_zpos_" << zpos << "_v4.root";      
...
//  gas->LoadGasFile("keith_co2_85_AR_15_T273.gas");
  ...
  SolidBox * enclosure = new SolidBox(0,0,0,8,40,10);
  ...
  */
  ...
  for (int iplane=0;iplane<2;iplane++){
    for(int ipad=0;ipad<nstrips_per_PCB;ipad++){
      ...
      /*cmp->AddWire(x-strip_size/4.0,y,strip_size/2.0,v,"T");
      cmp->AddWire(x+strip_size/4.0,y,strip_size/2.0,v,"T");
      std::cout << " wire " << x-strip_size/4.0 << " " << y << " " << v << " " << "T" << " count: " << count_me  << "out of " << nstrips << std::endl;
      std::cout << " wire " << x+strip_size/4.0 << " " << y << " " << v << " " << "T" << " count: " << count_me  << "out of " << nstrips << std::endl;

      cmp->AddWire(left_x-strip_size/4.0,y,strip_size/2.0,v,"T");
      cmp->AddWire(left_x+strip_size/4.0,y,strip_size/2.0,v,"T");
      std::cout << " wire " << left_x-strip_size/4.0 << " " << y << " " << v << " " << "T" << " count: " << count_me  << "out of " << nstrips << std::endl;
      std::cout << " wire " << left_x+strip_size/4.0 << " " << y << " " << v << " " << "T" << " count: " << count_me  << "out of " << nstrips << std::endl;
      */
    }
  }
  ...
  if (true) {
    ...
    //fieldView.SetElectricFieldRange(-1000., 1000.0);
    //fieldView.PlotProfile(-0.1, 1, 0.0,0.1, 1, 0.0, "ex"); // this line is for a sense wire region near (0,28.4,0).  // divide by 10.0 to get into centimeters
    //fieldView.PlotProfile(0, -33, 0.0,0, 33, 0.0, "ey"); // this line is for a sense wire region near (0,28.4,0).  // divide by 10.0 to get into centimeters
    //fieldView.PlotContour("ey"); // this line is for a sense wire region near (0,28.4,0).  // divide by 10.0 to get into centimeters
    //fieldView.SetElectricFieldRange(0, 2000.0);
    fieldView.SetArea(-7.7, -30.4, 7.7, 30.4);
    fieldView.PlotContour("V"); // this line is for a sense wire region near (0,28.4,0).  // divide by 10.0 to get into centimeters
    ...
    //fieldView->SaveAs("VoltageMapICRC2025.root");
  } else {
    ...
    cellView.SetArea(-8, 23,-1, 8, 31, 1.);
    cellView.Plot2d();
  }
