...
//  const double pressure = 1 * AtmosphericPressure; // in torr- we were at 14.6 psi (1 psi = 51.7149 torr)
  ...
  ingasfilename << "Flight2024_Boff_P_755.865_T_293.15_multiE_90CO2_10Ar_01122024.gas";
...
//  gas->LoadGasFile("Flight2024_Bon_P_755.865_T_298.15_.gas");
  ...
  const double max_y_i=12.8, max_x_i=max_x_in, stepy=0.2,stepx=0.5, min_y_i=-12.8;
  ...
  outputfilename << "flight_study_negative/isochronGenerator_multi_histo_xmin_" << x_i << "_xmax" << max_x_i << "_stepsize_"<< step_size << "_.txt";
  ...
  outrootfilename << "flight_study_negative/isochron_distr_" << x_i << "_" << max_x_i << "_" << step_size << "_.root";
  TFile * Outfile = new TFile(outrootfilename.str().c_str(),"recreate");
  // create histograms
  // = new TH1D("status","status",20,-19.5,0.5);
  // = new TH1D("drifttime_distr","drifttime_distr",1500,-0.5,14999.5);
  //TH1D * drifttime_h[size_of_vec_in_y];
  //for(int m=0;m<size_of_vec_in_y;m++){
  //  histonames.str( std::string());
  //  histonames.clear();
  // histonames << "driftTimeDistr_zmax_" << max_y_i*10.0 << "_zmin_" << min_y_i*10.0 << "_ymax_" << max_x_i*10.0 << "_ymin_" << min_x_in;
  //  drifttime_h[m] = new TH1D(histonames.str().c_str(),histonames.str().c_str(),1500,-0.5,14999.5);
  //}
...
//  Garfield::RandomEngine.setSeed(1);
  //RandomEngine();
  //rand.SetSeed(seed);
