...
void Plot_vdiff(){
  ...
  while(tilt_iter<4){
    TF1name.str("");
    TF1name << "Tilt_" << std::fixed << std::setprecision(0) << tilt << "um.root"; //Tilt_0um.root
    ...
    while (y_dist>0){
      ...
      if(tilt_iter>0) TransformAndSubtract(theseOG[tilt_iter][iter],theseOG[0][iter]);
      iter++;
      y_dist -=1.0;
    }
   // now we load in the other guys
     if(tilt_iter==0) tilt=100.0;
     else if(tilt_iter==1) tilt=200.0;
...
TCanvas *c1 = new TCanvas("c1", "MultiGraph Example", 800, 600);
    ...
    // Graph 4: Magenta
    theseOG[3][0]->SetLineColor(kMagenta);
    theseOG[3][0]->SetMarkerColor(kMagenta);
    theseOG[3][0]->SetMarkerStyle(23); // Inverted Triangle
//    theseOG[3][0]->SetTitle("Control Group");
    ...
    mg->Add(theseOG[3][0], "lp");

    // Set Global Titles (must be done before or during Draw)
    mg->SetTitle("At 10us drift, difference b/w drift distances ; z(cm); diff mm");
    ...
    //leg->AddEntry(theseOG[0][0], "0um", "lp");
    leg->AddEntry(theseOG[1][0], "100um", "lp");
    leg->AddEntry(theseOG[2][0], "200um", "lp");
    leg->AddEntry(theseOG[3][0], "400um", "lp");
    leg->Draw();

    // 8. Update and Save
    c1->SetGrid();
    c1->Update();
    c1->SaveAs("TiltMultiGraphOutput_driftdist_diff.pdf");
    _filem[0]->Close();
    _filem[1]->Close();
    _filem[2]->Close();
    _filem[3]->Close();
