...
void filter_flight_isochrones_inverted_283_303() {
  ...
  Target = TFile::Open( "temperature_study_isochrones_output_inverted_283_303.root", "RECREATE" );
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
  ...
  TH2D * isolow = new TH2D("iso283_inverted","iso283_inverted",6000,-14997.5,14997.5,289,-289,289);
  TH2D * isomid = new TH2D("iso293_inverted","iso293_inverted",6000,-14997.5,14997.5,289,-289,289);
  TH2D * isohigh = new TH2D("iso303_inverted","iso303_inverted",6000,-14997.5,14997.5,289,-289,289);

  for(int bint=1;bint<6001;bint++){
    for(int binz=1;binz<290;binz++){
