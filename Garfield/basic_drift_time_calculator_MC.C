...
//  const double pressure = 1 * AtmosphericPressure; // in torr- we were at 14.6 psi (1 psi = 51.7149 torr)
  ...
  //outfilename << "Flight2024_Boff_P_" << pressure <<"_T_" << temperature << "_.gas";
  outputfilename << "AvalMC_compare_pressures_A.txt";
...
//  gas->LoadGasFile("keith_co2_85_AR_15_T273.gas");
  ...
  //gas->LoadGasFile("Flight2024_Bon_P_755.865_T_303.15_90Ar_10CO2.gas");
  //gas->LoadGasFile("Flight2024_Bon_P_755.865_T_303.15_90Ar_10CO2_multiE.gas");
  //gas->LoadGasFile("Flight2024_Bon_P_0.517149_T_303.15_90Ar_10CO2_multiE100.gas");
  gas->LoadGasFile("Flight2024_Boff_P_759.997_T_293.15_95Ar_05CO2_Epoop2000.gas");
  ...
  for(int ix=0;ix<size_of_x_grid;ix++){
    ...
    std::vector<double> drift_v(size_of_vec_in_y);
    ...
    //cout << "Avalanch size limit is: " << (int) (aval->GetAvalancheSizeLimit()) << endl;
	...
	//bool did_it_drift=aval->AvalancheElectron(x_delt,y_delt,z_i,0,0,0,0,0);
      ...
      if(is_pos_borked){
        ...
        drift_v[iter_y]=x_delt/new_average;
    ...
    while(myval<iter_y){
      ...
      << "," << std_x[myval] << "," << std_y[myval] << "," << std_z[myval] << "," << e_a[myval] << "," << i_a[myval] << "," << drift_v[myval]
      << std::endl;
      myval++;
    }
