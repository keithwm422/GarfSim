...
void temperature_study_combine_moretemps() {
  ...
  Target = TFile::Open( "temperature_study_isochrones_output_moretemps.root", "RECREATE" );
...
void MergeRootfile( TDirectory *target, TList *sourcelist ) {
  ...
  TH2D * isolow = new TH2D("iso289","iso289",62,-77.5,77.5,578,-289,289);
  TH2D * isomid = new TH2D("iso293","iso293",62,-77.5,77.5,578,-289,289);  // 2.5 mm steps in y which is x axis of the TH2D
  TH2D * isohigh = new TH2D("iso297","iso297",62,-77.5,77.5,578,-289,289); //  1millimeter steps in z which is the y axis of this TH2D
  ...
  auto gmid = new TGraph2D();
  gmid->SetTitle("myisochrone293");
  gmid->SetName("myisochrone293");
  ...
  while (TObject *first_source_obj = next()){
    ...
    //skip status list that are bad
    std::string badnames = "status_e_z_197_y_17.5";
      ...
      if (strchr(key->GetName(), dritftime_find) != nullptr){
        ...
        if ( obj->IsA()->InheritsFrom( TH1D::Class() ) ) {
          ...
          if ( thish ) {
            ...
            if(thish->GetMean()>15000) cout << "AHHHH WTF" << endl;
            //target->cd();
            ...
            if(temperature>295){
              ...
              gmid->SetPoint(iter_i,std::stod(splitted[4]),std::stod(splitted[2]),thish->GetMean());
              isomid->Fill(std::stod(splitted[4]),std::stod(splitted[2]),thish->GetMean());
            }
        ...
        //std::cout << "Key name which is probably status: " << key->GetName() << " Type: " << key->GetClassName() << std::endl;
        //std::cout << "Key NBytes " << key->GetNbytes() << " Length " << key->GetObjlen() << " Sizeof key header " << key->Sizeof() << std::endl;
        //std::cout << "Key length: " << key->GetKeylen() << " Key Print " << endl;
        std::string thiskeyname(key->GetName());
        // /size_t found = thiskeyname.find(badnames);
        if(thiskeyname.find(badnames) != std::string::npos) continue;  // skip this one status_e_z_197_y_17.5
        key->Print();
        TObject *obj = key->ReadObj();
        if ( obj->IsA()->InheritsFrom( TH1D::Class() ) ) {
          TH1D * thish= (TH1D*)obj;
