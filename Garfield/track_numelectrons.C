...
int main(int argc, char * argv[]) {
  ...
  TH1D SingleWireDeltas("SingleWireDeltas","SingleWireDeltas",500,-0.5,1999.5); // in counts
...
//  gas->LoadGasFile("keith_co2_85_AR_15_T273.gas");
 ...
 sensor->AddElectrode(cmp,"a");
  ...
  if(zpos>0.5 && zpos <1.5){
    mass=8.4375e9; // in eV
    track.SetParticleUser(mass,4);    // for Be
    OutFile = new TFile("track_numelectrons_Be.root","recreate");
  }
  else if(zpos>1.5 && zpos <2.5){
    mass=3.752e9; // in eV
    track.SetParticleUser(mass,2);    // for He
    OutFile = new TFile("track_numelectrons_He.root","recreate");
  }
  else if(zpos>2.5 && zpos <3.5){
    track.SetParticle("proton");    // for He
    OutFile = new TFile("track_numelectrons_proton.root","recreate");
  }
  else if(zpos<0.5){
    track.SetParticle("muon");
    OutFile = new TFile("track_numelectrons_muon.root","recreate");
  }

  //track.SetParticle("mu-");
  track.SetBetaGamma(2.0);
  ...
  int num_tracks=500;
  for(int trki=0;trki<num_tracks;trki++){
