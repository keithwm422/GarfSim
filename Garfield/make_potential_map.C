...
int main(int argc, char * argv[]) {
  ...
  std::ofstream outputfilepot("potential_wires.txt");
  std::ofstream outputfiletweak("tweak_wires.txt");
  ...
  outputfilepot << "Potential Wire information" << std::endl;
  outputfiletweak << "Tweak Wire information" << std::endl;
...
//  cmp->SetMagneticField(0.,0.,0.0);
  ...
  /*for (int iplane=0;iplane<4;iplane++){
    for(int iw=0;iw<121;iw++){
      ...
      float x = (3.0*anode_cathode_sep)-((2.0*anode_cathode_sep)*iplane);
  ...
  for (int iplane=0;iplane<3;iplane++){
    for(int iw=0;iw<60;iw++){
      ...
      float sign = 1.0;
      if(iw%2==0) sign = -1.0;
      float x = (2.0*anode_cathode_sep)-iplane*(2.0*anode_cathode_sep)+sign*300e-4;
    ...
    for(int iw=0;iw<61;iw++){
      ...
      float x = (2.0*anode_cathode_sep)-iplane*(2.0*anode_cathode_sep);
      cmp->AddWire(x,y,2 * rPotential, vPotential, "p");
      outputfilepot << " wire " << x << " " << y << " " << vPotential << " " << "p" << std::endl;
    }
  }

  cmp->AddPlaneX(-3.0*anode_cathode_sep,-7500,"cP"); 
  //cmp->AddPlaneX(3.0*anode_cathode_sep,-7500,"cP");
  cmp->AddPlaneX(anode_cathode_sep,-7500,"cP");
  cmp->AddPlaneX(-1.0*anode_cathode_sep,-7500,"cP");
  std::cout << cmp->GetNumberOfPlanesX() << std::endl;
  float v;
  /*for (int iplane=0;iplane<2;iplane++){
    for(int iw=0;iw<241;iw++){
      float y = 24.5-49*iplane;
      float x = (3.0*anode_cathode_sep)-iw*cathodesep/2;
      if(x>2.0*anode_cathode_sep)
	{ 
	  v=vCathode + (vPotential-vCathode)*((3.0*anode_cathode_sep)-x)/(anode_cathode_sep);
	}
      else if(x<=2.0*anode_cathode_sep && x>anode_cathode_sep) 
	{
	  v=vPotential + (vCathode-vPotential)*((2.0*anode_cathode_sep)-x)/anode_cathode_sep;
	}
      else if(x<=anode_cathode_sep && x>0)
	{ 
	  v=vCathode + (vPotential-vCathode)*((anode_cathode_sep)-x)/anode_cathode_sep;
	}
      else if(x<=0 && x>-1.0*anode_cathode_sep) 
	{
	  v=vPotential + (vCathode-vPotential)*(-x)/anode_cathode_sep;
	}
      else if(x<=-1.0*anode_cathode_sep && x>-2.0*anode_cathode_sep)
	{ 
	  v=vCathode + (vPotential-vCathode)*((-1.0*anode_cathode_sep)-x)/anode_cathode_sep;
	}
      else if(x<=-2.0*anode_cathode_sep && x>-3.0*anode_cathode_sep) 
	{
	  v=vPotential + (vCathode-vPotential)*((-2.0*anode_cathode_sep)-x)/anode_cathode_sep;
	}
      //std::cout << " wire " << x << " " << y << " " << v << " " << "T" << std::endl;
    outputfiletweak << " wire " << x << " " << y << " " << v << " " << "T" << std::endl;
